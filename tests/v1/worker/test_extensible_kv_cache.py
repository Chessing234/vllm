...
def test_extensible_forces_block_outer_layout_for_hetero_pages():
    ...
    config generator must fall back to a block-outermost layout."""
    from types import SimpleNamespace

    import vllm.v1.attention.backends.utils as attn_backend_utils
    from vllm.v1.core.kv_cache_utils import _resolve_layout_for_groups
    ...
    homogeneous = [KVCacheGroupSpec(["layer.0"], small)]
    hetero = [
        KVCacheGroupSpec(["layer.0"], small),
        KVCacheGroupSpec(["layer.1"], big),
    ]

    def resolve(groups, extensible):
        prev = attn_backend_utils._RESOLVED_KV_CACHE_LAYOUT
        attn_backend_utils._RESOLVED_KV_CACHE_LAYOUT = KVCacheLayout.LBHNC
        cache_config = SimpleNamespace(
            enable_extensible_kv_cache=extensible, kv_cache_layout=None
        )
        try:
            return _resolve_layout_for_groups(groups, cache_config)
        finally:
            attn_backend_utils._RESOLVED_KV_CACHE_LAYOUT = prev

    # Homogeneous pages keep the layer-compact layout either way.
    assert resolve(homogeneous, True) is KVCacheLayout.LBHNC
    # Heterogeneous pages force block-outer under the extensible KV cache;
    # multiple overlaid groups force it regardless.
    assert resolve(hetero, True) is KVCacheLayout.BLHNC
    assert resolve(hetero, False) is KVCacheLayout.BLHNC
