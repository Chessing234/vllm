...
def test_multi_run_layer_compact_strides_place_hoisted_heads():
    ...
    the head planes sit between the layers and the blocks, so a run's block
    stride is one plane's slice, not the whole page (regression test for
    ...
    plane = full.block_size * full.state_content_size_bytes
    for tensor in config.kv_cache_tensors:
        # One plane per block, head planes between the layers and blocks.
        assert tensor.block_stride == plane
        assert tensor.layer_stride == full.num_heads * num_blocks * plane
        # The last block of the last layer's last head plane must stay
        # within the allocation (the old page-based stride overran it).
        end = (
            tensor.offset
            + tensor.layer_stride * (len(tensor.layers) - 1)
            + (full.num_heads - 1) * num_blocks * plane
