...
prepare_native_workspace() {
  ...
  if [[ -z "${BUILDKITE_BUILD_ID:-}" \
    || "${VLLM_CI_BASE_IMAGE}" != *":ci_base-build-${BUILDKITE_BUILD_ID}" ]]; then
    echo "ROCm native base is not scoped to Buildkite build ${BUILDKITE_BUILD_ID:-unset}: ${VLLM_CI_BASE_IMAGE}" >&2
    return 1
  fi
